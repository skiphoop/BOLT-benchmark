...
static int head=0;
static int tail=0;
static int init_size[5]={100000,500000,1000000,5000000,10000000};
...
std::vector<xrt::bo> generate_bos_host_key_value_seperate_complete(xrt::device &device,xrt::kernel &init_kernel, int init_size){
...
void run_init_and_chain_kernel_20_percent_hbm_with_probility(xrt::device &device, xrt::kernel &init_kernel,xrt::kernel &chain_kernel, std::vector<KV_TUPLE> init_tuples,std::vector<xrt::bo> bos,int init_size) {
    ...
    // std::vector<Command_Packet> commands=generate_commands_20_percent_hbm(command_size,init_tuples);
...
int main(int argc, char** argv) {
    ...
    for(int i=0;i<sizeof(init_size)/sizeof(int);i++){
        ...
        std::vector<KV_TUPLE> init_tuples=generate_distributed_init_tuples(init_size[i]);
        printf("Init tuples size is %ld\n",init_tuples.size());
        printf("The value size is %ld\n",VALUE_SIZE);
        auto bos=generate_bos_host_key_value_seperate_complete(device,init_kernel,init_size[i]);
        run_init_and_chain_kernel_20_percent_hbm_with_probility(device,init_kernel,chain_kernel,init_tuples,bos,init_size[i]);
    }
    return 0;
}