...
void run_init_and_chain_kernel_full_hbm_with_probility(xrt::device &device, xrt::kernel &init_kernel,xrt::kernel &chain_kernel, std::vector<KV_TUPLE> init_tuples,std::vector<xrt::bo> bos,int init_size) {
    ...
    // std::vector<Command_Packet> commands=generate_commands_full_hbm(command_size,init_tuples);
...
int main(int argc, char** argv) {
    ...
    for(int i=0;i<sizeof(init_size)/sizeof(int);i++){
        ...
        //test 100% hbm using one test function
        ...
        run_init_and_chain_kernel_full_hbm_with_probility(device,init_kernel,chain_kernel,init_tuples,bos,init_size[i]);
    }
    return 0;
}