...
void run_init_and_chain_kernel_50_percent_hbm_with_probility(xrt::device &device, xrt::kernel &init_kernel,xrt::kernel &chain_kernel, std::vector<KV_TUPLE> init_tuples,std::vector<xrt::bo> bos) {
    // Allocate memory dynamically
    Attribute (*position_map)[HBM_CAPACITY] = new Attribute[HASH_SIZE][HBM_CAPACITY];
    Value* value = new Value[INIT_SIZE];
    uint8_t* bitmap = new uint8_t[INIT_SIZE];
    ...
    //Generate Commands
    const int command_size=CMD_SIZE;
    // std::vector<Command_Packet> commands=generate_commands_50_percent_hbm(command_size,init_tuples);
    ...
    //         }
    //     }
    // }OEE-fpga/kernel/reconstruct/load_balance_dataflow_1d_array_50_percent_hbm_8_value_size.xclbin;
    // std::cout<<"The number of mistake is "<<mistake<<std::endl;
    // std::cout<<"Error casused by not inserted into the hbm or host page (random seed) is "<<not_insert_count<<std::endl;
    // std::cout<<"The number of get command is "<<get_cmd_count<<std::endl;
    // std::cout<<"The number of put command is "<<put_cmd_count<<std::endl;
...
int main(int argc, char** argv) {
    ...
    run_init_and_chain_kernel_50_percent_hbm_with_probility(device,init_kernel,chain_kernel,init_tuples,bos);
