...
void dummy_evict_page(const Page& page, const Eviction_Buffer_Block (*eviction_buffer)[PAGE_CAPACITY], const Attribute (*position_map)[HBM_CAPACITY], uint32_t page_index) {
    // Local arrays matching kernel exactly
    uint32_t eviction_buffer_key[8] = {0};
    uint32_t page_key[8] = {0};
    int eviction_buffer_to_page_empty_index[8] = {0};
    uint32_t eviction_buffer_to_position_map_index[8] = {0};
    
    // Local arrays to simulate write operations
    KV_TUPLE temp_page[8];
    Eviction_Buffer_Block temp_eviction_buffer[8];
    Attribute temp_position_map[HBM_CAPACITY];  // Changed size to match kernel
    
    // Load keys and values (full data copy)
    for(int i = 0; i < 8; i++) {
        page_key[i] = page.tuples[i].key;
        eviction_buffer_key[i] = eviction_buffer[page_index][i].key;
        // Full data copy to temp arrays
        temp_page[i] = page.tuples[i];
        temp_eviction_buffer[i] = eviction_buffer[page_index][i];
    }

    // Find empty slots (matches FIND_EVICTION_TO_PAGE_EMPTY_INDEX)
    for(int i = 0; i < 8; i++) {
        if(eviction_buffer_key[i] == static_cast<uint32_t>(EMPTY_KEY)) {
            eviction_buffer_to_page_empty_index[i] = -1;
        }
        else {
            int empty_index = -1;
            for(int j = 0; j < 8; j++) {
                if(page_key[j] == static_cast<uint32_t>(EMPTY_KEY) && empty_index == -1) {
                    empty_index = j;
                    page_key[j] = eviction_buffer_key[i];
                }
            }
            eviction_buffer_to_page_empty_index[i] = empty_index;
        }
    }

    // Find position map indices
    for(int i = 0; i < 8; i++) {
        if(eviction_buffer_to_page_empty_index[i] != -1) {
            uint32_t hash_index = eviction_buffer_key[i] % HASH_SIZE;
            const Attribute* hash_position_map = position_map[hash_index];
            for(int j = 0; j < HBM_CAPACITY; j++) {
                if(hash_position_map[j].key == eviction_buffer_key[i]) {
                    eviction_buffer_to_position_map_index[i] = j;
                    break;
                }
            }
        }
    ...
    // Simulate INSERT operation - modified for Eviction_Buffer_Block with memset
    for(int i = 0; i < 8; i++) {
        if(eviction_buffer_to_page_empty_index[i] != -1) {
            int empty_index = eviction_buffer_to_page_empty_index[i];
            
            // Set the key in the page
            temp_page[empty_index].key = temp_eviction_buffer[i].key;
            
            // Simulate value insertion with memset
            // This sets a dummy pattern (0xAA) in the value field
            memset(temp_page[empty_index].blocks, 0xAA, VALUE_SIZE);
            
            // Mark this slot in eviction buffer as empty
            temp_eviction_buffer[i].key = static_cast<uint32_t>(EMPTY_KEY);
        }
    }

    // Simulate UPDATE_POSITION_MAP operation
    for(int i = 0; i < 8; i++) {
        if(eviction_buffer_to_page_empty_index[i] != -1) {
            uint32_t hash_index = eviction_buffer_key[i] % HASH_SIZE;
            // Use stored index directly, matching kernel
            int position_map_index = eviction_buffer_to_position_map_index[i];
            temp_position_map[position_map_index].location_type = 1;
            temp_position_map[position_map_index].page_index = page_index;
            temp_position_map[position_map_index].location_index = eviction_buffer_to_page_empty_index[i];
        }
    }
}
                        ...
                        if(pages[page_idx].tuples[j].key == EMPTY_KEY) {
                            ...
                            for(int k = 0; k < HBM_CAPACITY; k++) {
                                if(position_map[hash_index][k].key == EMPTY_KEY) {
                                    ...
                                    // Add dummy evict page operation after successful page insertion
                                    // dummy_evict_page(pages[page_idx], eviction_buffer, position_map, page_idx);
                                    break;
                                }
                            }
                            break;
...
void run_init_and_chain_kernel_20_percent_hbm_with_probility(xrt::device &device, xrt::kernel &init_kernel, xrt::kernel &chain_kernel, std::vector<KV_TUPLE> init_tuples, std::vector<xrt::bo> bos) {
    // Allocate memory dynamically
    Attribute (*position_map)[HBM_CAPACITY] = new Attribute[HASH_SIZE][HBM_CAPACITY];
    Value_Block (*value_blocks)[BLOCK_SIZE] = new Value_Block[INIT_SIZE][BLOCK_SIZE];
    uint8_t* bitmap = new uint8_t[INIT_SIZE];
...
int main(int argc, char* argv[]) {
    ...
    run_init_and_chain_kernel_20_percent_hbm_with_probility(device, init_kernel, chain_kernel, init_tuples, bos);
