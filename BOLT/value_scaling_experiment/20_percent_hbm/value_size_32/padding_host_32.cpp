...
#define BLOCK_SIZE (PADDING_SIZE/16)
#define ACTUAL_BLOCK_SIZE (VALUE_SIZE/16)
using namespace std;
...
std::vector<KV_TUPLE> generate_distributed_init_tuples(uint32_t size) {
    ...
    while(tuples.size() < size) {    
        ...
        if(used_keys.insert(key).second) {
            tuple.key = key;
            for(int b = 0; b < ACTUAL_BLOCK_SIZE; b++) {
                for(int i = 0; i < 16; i++) {
...
int main(int argc, char* argv[]) {
    ...
    printf("The padding size is %ld\n",PADDING_SIZE);
    printf("The block size is %ld\n",BLOCK_SIZE);
    printf("The actual block size is %ld\n",ACTUAL_BLOCK_SIZE);
