...
void run_init_and_chain_kernel_full_hbm_with_probility(xrt::device &device, xrt::kernel &init_kernel,xrt::kernel &chain_kernel, std::vector<KV_TUPLE> init_tuples,std::vector<xrt::bo> bos) {
    // Allocate memory dynamically
    Attribute (*position_map)[HBM_CAPACITY] = new Attribute[HASH_SIZE][HBM_CAPACITY];
    Value* value = new Value[INIT_SIZE];
    uint8_t* bitmap = new uint8_t[INIT_SIZE];
...
int main(int argc, char** argv) {
    ...
    run_init_and_chain_kernel_full_hbm_with_probility(device,init_kernel,chain_kernel,init_tuples,bos);
